...
async def work_experience_2_node(state: AgentState):
    """
    Agent that updates Work Experience 2 section based on the Job Description.
    """
    print("--- UPDATING WORK EXPERIENCE 2 ---")

    # 1. Read the original experience
    exp_path = settings.get_path("resumeSections", "experience2.tex")
    ...
    updated_path = settings.get_path("updatedResumeSections", "experience2.tex")
    with open(updated_path, "w", encoding="utf-8") as f:
        f.write(updated_content)

    # Update state
    state["resume_sections"]["work_experience_2"] = updated_content
